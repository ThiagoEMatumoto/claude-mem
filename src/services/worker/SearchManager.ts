...
export class SearchManager {
	...
	async search(args: any): Promise<any> {
		...
		const { query, type, obs_type, concepts, files, format, ...options } =
			normalized;
		...
		if (!query) {
			logger.debug(
				"SEARCH",
				"Filter-only query (no query text), using direct SQLite filtering",
				{ enablesDateFilters: true },
			);
			const obsOptions = { ...options, type: obs_type, concepts, files };
			if (searchObservations) {
				observations = this.sessionSearch.searchObservations(
					undefined,
					obsOptions,
				);
			}
		...
		// PATH 2: CHROMA SEMANTIC SEARCH (query text + Chroma available)
			...
			logger.debug("SEARCH", "Using ChromaDB semantic search", {
				typeFilter: type || "all",
			});
			...
			logger.debug("SEARCH", "ChromaDB returned semantic matches", {
			...
			if (chromaResults.ids.length > 0) {
				...
				logger.debug("SEARCH", "Results within 90-day window", {
					count: recentMetadata.length,
				});
				...
				if (obsIds.length > 0) {
					...
					const obsOptions = { ...options, type: obs_type, concepts, files };
					observations = this.sessionStore.getObservationsByIds(
						obsIds,
						obsOptions,
					);
				}
				...
				logger.debug(
					"SEARCH",
					"ChromaDB found no matches (final result, no FTS5 fallback)",
					{},
				);
			}
		}
		// ChromaDB not initialized - mark as failed to show proper error message
			...
			logger.debug(
				"SEARCH",
				"ChromaDB not initialized - semantic search unavailable",
				{},
			);
		...
		const limitedResults = allResults.slice(0, options.limit || 50);
			...
			for (const result of dayResults) {
				...
				if (result.type === "observation") {
					file = extractFirstFile(
						result.data.files_modified,
						cwd,
						result.data.files_read,
					);
				}
			...
			for (const [file, fileResults] of resultsByFile) {
				...
				for (const result of fileResults) {
					if (result.type === "observation") {
						...
						const formatted = this.formatter.formatUserPromptSearchRow(
	...
	async timeline(args: any): Promise<any> {
		...
		if (query) {
			...
			if (this.chromaSync) {
				try {
					logger.debug(
						"SEARCH",
						"Using hybrid semantic search for timeline query",
						{},
					);
					const chromaResults = await this.queryChroma(query, 100);
					logger.debug(
						"SEARCH",
						"Chroma returned semantic matches for timeline",
			...
			logger.debug(
				"SEARCH",
				"Query mode: Using observation as timeline anchor",
				{ observationId: topResult.id },
			);
		...
		// MODE 2: Anchor-based timeline
			...
			if (anchor.startsWith("S") || anchor.startsWith("#S")) {
				...
				const sessions = this.sessionStore.getSessionSummariesByIds([
					sessionNum,
				]);
				...
				timelineData = this.sessionStore.getTimelineAroundTimestamp(
					anchorEpoch,
					depth_before,
					depth_after,
					project,
		...
		const filteredItems = this.timelineService.filterByDepth(
			items,
			anchorId,
			anchorEpoch,
			depth_before,
		...
		// Header
		if (query) {
			const anchorObs = filteredItems.find(
				(item) => item.type === "observation" && item.data.id === anchorId,
			);
			const anchorTitle =
		...
		for (const [day, dayItems] of sortedDays) {
			...
			for (const item of dayItems) {
				...
				if (item.type === "session") {
					...
					lines.push(
						`**\uD83C\uDFAF #S${sess.id}** ${title} (${formatDateTime(item.epoch)})${marker}`,
					);
					lines.push("");
					...
					lines.push(
						`**\uD83D\uDCAC User Prompt #${prompt.prompt_number}** (${formatDateTime(item.epoch)})`,
					);
					lines.push(`> ${truncated}`);
					lines.push("");
				} else if (item.type === "observation") {
					const obs = item.data as ObservationSearchResult;
					const file = extractFirstFile(
						obs.files_modified,
						cwd,
						obs.files_read,
					);
					...
					lines.push(
						`| #${obs.id} | ${timeDisplay} | ${icon} | ${title}${anchorMarker} | ~${tokens} |`,
					);
				}
			}
	...
	async decisions(args: any): Promise<any> {
		...
		if (this.chromaSync) {
			try {
				if (query) {
					...
					if (obsIds.length > 0) {
						results = this.sessionStore.getObservationsByIds(obsIds, {
							...filters,
							type: "decision",
						});
					...
					const metadataResults = this.sessionSearch.findByType(
						"decision",
						filters,
					);
		...
		const formattedResults = results.map((obs, i) =>
			this.formatter.formatObservationIndex(obs, i),
		);
	...
	async changes(args: any): Promise<any> {
		...
		if (this.chromaSync) {
			try {
				logger.debug(
					"SEARCH",
					"Using hybrid search for change-related observations",
					{},
				);
				...
				const conceptChangeResults = this.sessionSearch.findByConcept(
					"change",
					filters,
				);
				const conceptWhatChangedResults = this.sessionSearch.findByConcept(
					"what-changed",
					filters,
				);
				...
				[
					...typeResults,
					...conceptChangeResults,
					...conceptWhatChangedResults,
				].forEach((obs) => allIds.add(obs.id));
		...
		if (results.length === 0) {
			const typeResults = this.sessionSearch.findByType("change", filters);
			const conceptResults = this.sessionSearch.findByConcept(
				"change",
				filters,
			);
			const whatChangedResults = this.sessionSearch.findByConcept(
				"what-changed",
				filters,
			);

			const allIds = new Set<number>();
			[...typeResults, ...conceptResults, ...whatChangedResults].forEach(
				(obs) => allIds.add(obs.id),
			);
	...
	async howItWorks(args: any): Promise<any> {
		...
		if (this.chromaSync) {
			logger.debug(
				"SEARCH",
				"Using metadata-first + semantic ranking for how-it-works",
				{},
			);
			const metadataResults = this.sessionSearch.findByConcept(
				"how-it-works",
				filters,
			);
	...
	async searchObservations(args: any): Promise<any> {
		...
		if (this.chromaSync) {
			logger.debug(
				"SEARCH",
				"Using hybrid semantic search (Chroma + SQLite)",
				{},
			);

			// Step 1: Chroma semantic search (top 100)
			const chromaResults = await this.queryChroma(query, 100);
			logger.debug("SEARCH", "Chroma returned semantic matches", {
				matchCount: chromaResults.ids.length,
			});
			...
			if (chromaResults.ids.length > 0) {
				...
				if (recentIds.length > 0) {
					...
					logger.debug("SEARCH", "Hydrated observations from SQLite", {
	...
	async searchSessions(args: any): Promise<any> {
		...
		if (this.chromaSync) {
			...
			logger.debug("SEARCH", "Chroma returned semantic matches for sessions", {
				matchCount: chromaResults.ids.length,
			});
			...
			if (chromaResults.ids.length > 0) {
				...
				if (recentIds.length > 0) {
					const limit = options.limit || 20;
					results = this.sessionStore.getSessionSummariesByIds(recentIds, {
						orderBy: "date_desc",
						limit,
					});
					logger.debug("SEARCH", "Hydrated sessions from SQLite", {
						count: results.length,
					});
				}
			}
		...
		const formattedResults = results.map((session, i) =>
			this.formatter.formatSessionIndex(session, i),
		);
	...
	async searchUserPrompts(args: any): Promise<any> {
		...
		if (this.chromaSync) {
			logger.debug(
				"SEARCH",
				"Using hybrid semantic search for user prompts",
				{},
			);
			...
			logger.debug("SEARCH", "Chroma returned semantic matches for prompts", {
				matchCount: chromaResults.ids.length,
			});
			...
			if (chromaResults.ids.length > 0) {
				...
				if (recentIds.length > 0) {
					...
					logger.debug("SEARCH", "Hydrated user prompts from SQLite", {
						count: results.length,
					});
				}
			}
				...
				content: [
					{
						type: "text" as const,
						text: query
							? `No user prompts found matching "${query}"`
		...
		const formattedResults = results.map((prompt, i) =>
			this.formatter.formatUserPromptIndex(prompt, i),
		);
	...
	async findByConcept(args: any): Promise<any> {
		...
		if (this.chromaSync) {
			logger.debug(
				"SEARCH",
				"Using metadata-first + semantic ranking for concept search",
				{},
			);
			...
			logger.debug("SEARCH", "Found observations with concept", {
				concept,
			...
			if (metadataResults.length > 0) {
				...
				logger.debug("SEARCH", "Chroma ranked results by semantic relevance", {
	...
	async findByFile(args: any): Promise<any> {
		...
		if (this.chromaSync) {
			logger.debug(
				"SEARCH",
				"Using metadata-first + semantic ranking for file search",
				{},
			);
			...
			if (metadataResults.observations.length > 0) {
				...
				logger.debug(
					"SEARCH",
					"Chroma ranked observations by semantic relevance",
					{ count: rankedIds.length },
				);

				// Step 3: Hydrate in semantic rank order
				if (rankedIds.length > 0) {
					observations = this.sessionStore.getObservationsByIds(rankedIds, {
						limit: filters.limit || 20,
					});
					// Restore semantic ranking order
					observations.sort(
		...
		for (const [day, dayResults] of resultsByDate) {
			...
			for (const result of dayResults) {
				if (result.type === "observation") {
					lines.push(
						this.formatter.formatObservationIndex(
							result.data as ObservationSearchResult,
							0,
						),
					);
					...
					lines.push(
						this.formatter.formatSessionIndex(
							result.data as SessionSummarySearchResult,
							0,
						),
					);
	...
	async getRecentContext(args: any): Promise<any> {
		...
		const sessions = this.sessionStore.getRecentSessionsWithStatus(
			project,
			limit,
		);
		...
		lines.push("");
		lines.push(
			`Showing last ${sessions.length} session(s) for **${project}**:`,
		);
		lines.push("");
		...
		for (const session of sessions) {
			...
			if (session.has_summary) {
				const summary = this.sessionStore.getSummaryForSession(
					session.memory_session_id,
				);
				if (summary) {
					const promptLabel = summary.prompt_number
					...
					if (summary.next_steps)
						lines.push(`**Next Steps:** ${summary.next_steps}`);
					...
					if (summary.files_read) {
						try {
							...
							logger.debug(
								"WORKER",
								"files_read is plain string, using as-is",
								{},
								error as Error,
							);
					...
					if (summary.files_edited) {
						try {
							...
							logger.debug(
								"WORKER",
								"files_edited is plain string, using as-is",
								{},
								error as Error,
							);
				...
				const observations = this.sessionStore.getObservationsForSession(
					session.memory_session_id,
				);
	...
	async getTimelineByQuery(args: any): Promise<any> {
		...
		if (mode === "interactive") {
			...
			logger.debug(
				"SEARCH",
				"Auto mode: Using observation as timeline anchor",
				{ observationId: topResult.id },
			);
			...
			lines.push(`# Timeline for query: "${query}"`);
			lines.push(
				`**Anchor:** Observation #${topResult.id} - ${topResult.title || "Untitled"}`,
			);
			lines.push(
				`**Window:** ${depth_before} records before -> ${depth_after} records after | **Items:** ${filteredItems?.length ?? 0}`,
			);
			...
			for (const [day, dayItems] of sortedDays) {
				...
				for (const item of dayItems) {
					const isAnchor =
						item.type === "observation" && item.data.id === topResult.id;
					...
					if (item.type === "session") {
						...
						lines.push(
							`**\uD83C\uDFAF #S${sess.id}** ${title} (${formatDateTime(item.epoch)})`,
						);
						lines.push("");
					} else if (item.type === "prompt") {
						// Close any open table
